...
class RealVoiceCorrectionTester:
    ...
    async def test_voice_correction_pipeline(
        ...
        try:
            ...
            corrected_result = await voice_cloning_service.generate_corrected_pronunciation(
                user_id=self.user_id,
                target_text=target_text,
            ...
            results["steps"]["corrected_generation"] = {
                ...
                "corrections_applied": corrected_result.get("corrections_applied", [])
            }
            ...
            if corrected_audio_base64:
                ...
                corrected_file = f"corrected_{self.user_id}_{timestamp}.wav"
            ...
            corrected_stt = await stt_service.recognize_from_base64(
                audio_base64=corrected_audio_base64,
                language=f"{language}-{language.upper()}" if language == "ko" else f"{language}-US"
            )
            
            corrected_stt_time = time.time() - corrected_stt_start
            
            results["steps"]["corrected_stt"] = {
                "recognized_text": corrected_stt or "인식 실패",
                "time_ms": corrected_stt_time * 1000,
                "success": corrected_stt is not None
            }
            
            print(f"✅ 교정된 음성 STT 결과: '{corrected_stt}'")
    ...
    def generate_test_report(self, results: Dict):
        ...
        if results.get("corrected_audio_file"):
            print(f"🎵 교정된 음성 파일: {results['corrected_audio_file']}")
